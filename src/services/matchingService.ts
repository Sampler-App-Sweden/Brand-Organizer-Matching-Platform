...
export const calculateMatchScore = (brand: any, organizer: any): MatchScore => {
  let score = 0;
  const reasons: string[] = [];
  // Target audience matching
  if (brand.targetAudience && organizer.audienceDescription && hasKeywordMatch(brand.targetAudience, organizer.audienceDescription)) {
    score += 20;
    reasons.push('Audience alignment');
  }
  // Age demographics matching
  if (brand.ageRange && organizer.audienceDemographics && hasAgeRangeOverlap(brand.ageRange, organizer.audienceDemographics)) {
    score += 15;
    reasons.push('Demographic fit');
  }
  // Industry relevance
  if (brand.industry && organizer.eventType && isIndustryRelevant(brand.industry, organizer.eventType)) {
    score += 15;
    reasons.push('Industry relevance');
  }
  // Marketing goals alignment with event offerings
  if (brand.marketingGoals && organizer.offeringTypes && hasMarketingGoalMatch(brand.marketingGoals, organizer.offeringTypes)) {
    score += 25;
    reasons.push('Marketing goals alignment');
  }
  // Budget appropriateness for event size
  if (brand.budget && organizer.attendeeCount && isBudgetAppropriate(brand.budget, organizer.attendeeCount)) {
    score += 15;
    reasons.push('Budget fit');
  }
  // Sponsorship type match
  if (brand.sponsorshipType && organizer.sponsorshipNeeds && hasSponsorshipTypeMatch(brand.sponsorshipType, organizer.sponsorshipNeeds)) {
    score += 10;
...
const hasKeywordMatch = (text1: string, text2: string): boolean => {
  ...
  return keywords1.some(word => keywords2.includes(word) && word.length > 3 && !commonWords.includes(word));
};
const hasAgeRangeOverlap = (brandAgeRange: string | string[], organizerDemographics: string | string[]): boolean => {
  const brandAges = Array.isArray(brandAgeRange) ? brandAgeRange : [brandAgeRange];
  const organizerAges = Array.isArray(organizerDemographics) ? organizerDemographics : [organizerDemographics];
...
const isBudgetAppropriate = (budget: string, attendeeCount: string): boolean => {
...
const hasSponsorshipTypeMatch = (brandTypes: string[], organizerNeeds: string): boolean => {
  ...
  return brandTypes.some(type => {
    ...
    return keywords.some(keyword => organizerNeeds.toLowerCase().includes(keyword));
  });
};
// Common words to ignore in matching
const commonWords = ['and', 'the', 'for', 'with', 'that', 'this', 'have', 'from', 'they', 'will', 'would', 'about', 'there', 'their', 'what', 'when', 'make', 'like', 'time', 'just', 'know', 'people', 'year', 'your', 'good', 'some', 'could', 'them', 'than', 'then', 'look', 'only', 'come', 'over', 'think', 'also', 'back', 'after', 'work', 'first', 'well', 'even', 'want', 'because', 'these', 'give'];
// Function to find matches for a brand
export const findMatchesForBrand = (brand: any, organizers: any[]): Match[] => {
  return organizers.map(organizer => {
    const {
      score,
      reasons
    } = calculateMatchScore(brand, organizer);
...
export const findMatchesForOrganizer = (organizer: any, brands: any[]): Match[] => {
