...
export const getOrCreateConversation = (brandId: string, organizerId: string): Conversation => {
  const conversations = JSON.parse(localStorage.getItem('conversations') || '[]') as Conversation[];
  let conversation = conversations.find(c => c.brandId === brandId && c.organizerId === organizerId);
  if (!conversation) {
    conversation = {
...
export const sendMessage = (conversationId: string, senderId: string, senderType: 'brand' | 'organizer', content: string): Message => {
  const conversations = JSON.parse(localStorage.getItem('conversations') || '[]') as Conversation[];
  ...
  if (content.toLowerCase().includes('?') || content.toLowerCase().includes('help') || content.toLowerCase().includes('suggest')) {
    const aiResponse = generateAIResponse(content, senderType, conversations[conversationIndex]);
...
export const getOrganizerConversations = (organizerId: string): Conversation[] => {
  const conversations = JSON.parse(localStorage.getItem('conversations') || '[]') as Conversation[];
  return conversations.filter(c => c.organizerId === organizerId);
};
// Generate AI response based on message content
const generateAIResponse = (message: string, senderType: 'brand' | 'organizer', conversation: Conversation): string => {
  ...
  // Timeline questions
  if (lowercaseMessage.includes('when') || lowercaseMessage.includes('timeline') || lowercaseMessage.includes('deadline')) {
  ...
  if (lowercaseMessage.includes('benefit') || lowercaseMessage.includes('value') || lowercaseMessage.includes('offer')) {
    if (senderType === 'brand') {
  ...
  // Logistics questions
  if (lowercaseMessage.includes('logistics') || lowercaseMessage.includes('setup') || lowercaseMessage.includes('requirements')) {
  ...
  if (lowercaseMessage.includes('contract') || lowercaseMessage.includes('terms') || lowercaseMessage.includes('agreement')) {
