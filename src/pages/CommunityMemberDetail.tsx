...
export function CommunityMemberDetail() {
  ...
  const canInteract = () => {
    ...
    return currentUser.type === 'brand' && member.type === 'organizer' || currentUser.type === 'organizer' && member.type === 'brand';
  };
  ...
  return <Layout>
      <div className="bg-white min-h-screen">
        ...
        <div className="container mx-auto px-4 py-6">
          <Link to="/community" className="inline-flex items-center text-indigo-600 hover:text-indigo-800">
          ...
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-6 md:p-8 mb-8 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start">
              <div className="w-32 h-32 md:w-40 md:h-40 mb-4 md:mb-0 md:mr-6 relative">
                {member.logoUrl ? <img src={member.logoUrl} alt={member.name} className="w-full h-full object-contain rounded-lg shadow-md border border-gray-100 bg-white p-2" /> : <div className={`w-full h-full rounded-lg flex items-center justify-center text-white font-bold text-4xl ${member.type === 'brand' ? 'bg-blue-500' : 'bg-purple-500'}`}>
                    {member.name.charAt(0)}
                  </div>}
                {member.featured && <div className="absolute -top-2 -right-2 bg-yellow-400 text-yellow-800 rounded-full p-1.5" title="Featured Member">
              ...
              <div className="text-center md:text-left md:flex-1">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <div className="flex items-center justify-center md:justify-start mb-3">
                      <span className={`text-sm px-3 py-1 rounded-full ${member.type === 'brand' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>
                  ...
                  <div className="mt-4 md:mt-0">
                    <Button variant="primary" className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 group relative overflow-hidden" onClick={handleCtaClick}>
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-4">
                  {member.website && <a href={member.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-sm text-indigo-600 hover:text-indigo-800">
                      <GlobeIcon className="h-4 w-4 mr-1" />
                      Website
                      <ExternalLinkIcon className="h-3 w-3 ml-1" />
                    </a>}
                  {member.email && <a href={`mailto:${member.email}`} className="inline-flex items-center text-sm text-indigo-600 hover:text-indigo-800">
                      <MailIcon className="h-4 w-4 mr-1" />
                      Email
                    </a>}
                  {member.phone && <a href={`tel:${member.phone}`} className="inline-flex items-center text-sm text-indigo-600 hover:text-indigo-800">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 mb-8 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-100 via-purple-200 to-indigo-100"></div>
                <h2 className="text-xl font-semibold text-gray-900 mb-4">
                  {member.type === 'brand' ? 'What They Offer' : 'Events & Opportunities'}
                </h2>
                {member.type === 'brand' ? <div className="space-y-4">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 mb-8 relative overflow-hidden">
                ...
                <div className="mt-6">
                  <Button variant="primary" className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 group relative overflow-hidden" onClick={handleCtaClick}>
              ...
              {/* Social Media Links */}
              {member.socialLinks && <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-100 via-purple-200 to-indigo-100"></div>
                  <h2 className="text-xl font-semibold text-gray-900 mb-4">
                    Connect
                  </h2>
                  <div className="space-y-2">
                    {member.socialLinks.split('\n').map((link, index) => <a key={index} href={link} target="_blank" rel="noopener noreferrer" className="flex items-center text-indigo-600 hover:text-indigo-800">
      ...
      {showWizard && <InterestOfferWizard member={member} currentUser={currentUser} onClose={() => setShowWizard(false)} />}
    </Layout>;
}
