...
export function OrganizerSponsorshipPanel() {
  ...
  const handleAllocationChange = (type: 'product' | 'discount' | 'financial', value: number) => {
    const remaining = 100 - value;
    if (type === 'product') {
      // Distribute remaining percentage between discount and financial
      const discountShare = Math.round(remaining * (allocation.discount / (allocation.discount + allocation.financial)));
      setAllocation({
        product: value,
      ...
      const productShare = Math.round(remaining * (allocation.product / (allocation.product + allocation.financial)));
      setAllocation({
      ...
      const productShare = Math.round(remaining * (allocation.product / (allocation.product + allocation.discount)));
      setAllocation({
        product: productShare,
  ...
  return <div className="bg-white rounded-lg shadow-sm p-6 border border-indigo-100 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden opacity-5 pointer-events-none">
        {[...Array(50)].map((_, i) => <div key={i} className="absolute rounded-full bg-white" style={{
        top: `${Math.random() * 100}%`,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        {sponsorshipRequests.map(type => <div key={type.id} className={`relative rounded-lg p-5 cursor-pointer transition-all duration-300 ${selectedTypes.includes(type.id) ? 'bg-blue-50 border-2 border-blue-300' : 'bg-white border border-gray-200 hover:border-blue-200'}`} onClick={() => handleTypeToggle(type.id)}>
            <div className="absolute top-3 right-3">
              <div className={`h-5 w-5 rounded-full ${selectedTypes.includes(type.id) ? 'bg-blue-500' : 'bg-gray-200'} flex items-center justify-center`}>
                {selectedTypes.includes(type.id) && <div className="h-2 w-2 bg-white rounded-full"></div>}
              </div>
            </div>
            <div className="flex items-start">
              <div className={`p-3 rounded-full ${selectedTypes.includes(type.id) ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-500'} mr-4`}>
                {type.icon}
              </div>
              <div>
      ...
      {selectedTypes.length > 0 && <div className="space-y-8 mb-8">
          {selectedTypes.includes('product') && <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="e.g. Coffee beans, T-shirts, Water bottles" value={productDetails.items} onChange={e => setProductDetails({
              ...productDetails,
              items: e.target.value
                ...
                <div>
                  ...
                  <input type="number" className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="e.g. 200" value={productDetails.quantity} onChange={e => setProductDetails({
              ...productDetails,
          ...
          {selectedTypes.includes('discount') && <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="e.g. 15" value={discountDetails.targetLevel} onChange={e => setDiscountDetails({
              ...discountDetails,
              targetLevel: e.target.value
                ...
                <div>
                  ...
                  <input type="number" className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="e.g. 500" value={discountDetails.expectedVolume} onChange={e => setDiscountDetails({
              ...discountDetails,
          ...
          {selectedTypes.includes('financial') && <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="number" className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="e.g. 500" value={financialDetails.minAmount} onChange={e => setFinancialDetails({
              ...financialDetails,
              minAmount: e.target.value
                ...
                <div>
                  ...
                  <input type="number" className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" placeholder="e.g. 2000" value={financialDetails.maxAmount} onChange={e => setFinancialDetails({
              ...financialDetails,
              maxAmount: e.target.value
                ...
                <div>
                  ...
                  <select className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" value={financialDetails.paymentWindow} onChange={e => setFinancialDetails({
              ...financialDetails,
          ...
          {selectedTypes.includes('any') && <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 relative">
              <div className="absolute -top-3 -left-3 w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
                <div className="h-3 w-3 text-blue-600" />
              </div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input type="range" min="0" max="100" value={allocation.product} onChange={e => handleAllocationChange('product', parseInt(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    <label className="text-sm font-medium text-gray-700 flex items-center">
                      <PercentIcon className="h-4 w-4 mr-1 text-blue-500" />{' '}
                  ...
                  <input type="range" min="0" max="100" value={allocation.discount} onChange={e => handleAllocationChange('discount', parseInt(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                </div>
                <div>
                  <div className="flex justify-between mb-1">
                    <label className="text-sm font-medium text-gray-700 flex items-center">
                      <DollarSignIcon className="h-4 w-4 mr-1 text-blue-500" />{' '}
                  ...
                  <input type="range" min="0" max="100" value={allocation.financial} onChange={e => handleAllocationChange('financial', parseInt(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                </div>
              </div>
            </div>}
        </div>}
      {/* Match preview panel */}
      {selectedTypes.length > 0 && <div className="bg-blue-50 p-6 rounded-lg border border-blue-100 mb-8 relative overflow-hidden">
          ...
          <div className="absolute bottom-4 right-4 text-blue-200 opacity-30 flex">
            ...
            <div className="h-1 w-1 rounded-full bg-blue-400 mr-3 mt-1"></div>
            <div className="h-2 w-2 rounded-full bg-blue-400 mr-1"></div>
            <div className="h-1 w-1 rounded-full bg-blue-400 mr-1 mt-1"></div>
            <div className="h-2 w-2 rounded-full bg-blue-400"></div>
          </div>
        </div>}
      {/* Action buttons */}
      <div className="flex justify-end space-x-4">
        <Button variant="outline" className="flex items-center hover:bg-blue-50 transition-colors" onClick={handleSaveDraft} disabled={isSubmitting || selectedTypes.length === 0}>
          <SaveIcon className="h-4 w-4 mr-2" />
          Save Draft
        </Button>
        <Button variant="primary" className="flex items-center bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 border-0 shadow-md hover:shadow-lg transition-all" onClick={handlePublish} disabled={isSubmitting || selectedTypes.length === 0}>
