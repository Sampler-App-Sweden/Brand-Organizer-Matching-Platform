...
import { FlaskConicalIcon, Sparkles, ArrowRightIcon, RotateCwIcon } from 'lucide-react';
...
export function CommunityShowcase() {
  ...
  return <section className="py-16 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        {[...Array(20)].map((_, i) => <div key={i} className="absolute h-1 w-1 bg-white rounded-full" style={{
        top: `${Math.random() * 100}%`,
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-12">
          ...
          <button onClick={refreshOrder} className="mt-4 md:mt-0 flex items-center text-indigo-600 hover:text-indigo-800 transition-colors">
        ...
        {loading && members.length === 0 ? <div className="flex justify-center items-center py-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {members.map(member => <CommunityCard key={member.id} member={member} />)}
            </div>
        ...
        <div className="mt-12 flex flex-col items-center">
          {hasMore && <Button variant="outline" className="flex items-center" onClick={loadMore} disabled={loading}>
              {loading ? <>
