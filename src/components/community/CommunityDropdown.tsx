...
export function CommunityDropdown({
  ...
  return <div className={`bg-white rounded-md shadow-lg overflow-hidden ${isMobile ? 'w-full' : 'w-[600px]'}`}>
      ...
      {loading ? <div className="p-8 text-center">
          ...
          <div className={`grid ${isMobile ? 'grid-cols-2' : 'grid-cols-4'} gap-4 p-4`}>
            {members.map(member => <div key={member.id} className="group relative">
                <Link to={`/community/${member.id}`} className="block p-3 rounded-md transition-all hover:bg-indigo-50">
                  <div className="h-12 w-12 mx-auto mb-2 relative">
                    {member.logoUrl ? <img src={member.logoUrl} alt={member.name} className="h-full w-full object-contain rounded-md group-hover:shadow-[0_0_10px_rgba(99,102,241,0.5)] transition-all" /> : <div className={`h-full w-full rounded-md flex items-center justify-center text-white font-bold text-xl ${member.type === 'brand' ? 'bg-blue-500' : 'bg-purple-500'}`}>
                        {member.name.charAt(0)}
                      </div>}
                    <div className={`absolute -top-1 -right-1 h-3 w-3 rounded-full ${member.type === 'brand' ? 'bg-blue-500' : 'bg-purple-500'}`}></div>
                  </div>
          ...
          <div className="p-3 bg-gray-50 text-center border-t border-gray-100">
            <Link to="/community" className="text-indigo-600 hover:text-indigo-800 text-sm font-medium flex items-center justify-center">
