...
export function RegistrationDebugHelper() {
  ...
  if (!isVisible) {
    return <button onClick={() => setIsVisible(true)} className="fixed bottom-4 right-4 bg-gray-800 text-white px-3 py-1 rounded-md text-sm opacity-50 hover:opacity-100">
        Debug
      </button>;
  }
  return <div className="fixed bottom-4 right-4 max-w-md w-full bg-white border border-gray-200 rounded-lg shadow-lg p-4 text-sm">
