...
export function AIOnboardingAssistant() {
  ...
  >([
    {
      role: 'assistant',
      content:
        "Welcome to SponsrAI! Tell me what you're looking for, and I'll help you get started."
    }
  ])
  ...
  useEffect(() => {
    const loadDraft = async () => {
      ...
      if (savedDraft) {
        ...
        const filledFields = Object.entries(savedDraft.profile).filter(
          ([key, value]) =>
            value !== undefined && value !== '' && requiredFields.includes(key)
        ).length
  ...
  const handleUserInput = async (input: string) => {
    ...
    try {
      ...
      const role =
        inferredRole || response.inferredRole || (await inferUserRole(input))
      if (role && !inferredRole) {
        ...
        setConversation((prev) => [
          ...
          {
            role: 'assistant',
            content: `I understand you're a ${getRoleName(
              role
      ...
      if (role) {
        ...
        const filledFields = Object.entries(updatedProfile).filter(
          ([key, value]) =>
            value !== undefined && value !== '' && requiredFields.includes(key)
        ).length
        setCompletionPercentage(
  ...
  const handleSwitchRole = (newRole: UserRole) => {
    ...
    setConversation((prev) => [
      ...
      {
        role: 'assistant',
        content: `I'll help you create a ${getRoleName(
          newRole
        )} profile instead. Let's build on what you've already shared.`
      }
    ])
    // Recalculate completion percentage for new role
    if (draftProfile) {
      const requiredFields = getRequiredFieldsForRole(newRole)
      const filledFields = Object.entries(draftProfile).filter(
        ([key, value]) =>
          value !== undefined && value !== '' && requiredFields.includes(key)
      ).length
      setCompletionPercentage(
        ...
        if (currentUser) {
          ...
          setConversation((prev) => [
            ...prev,
            {
              role: 'assistant',
              content:
                "To save your progress, you'll need to create an account or log in. Would you like to do that now?"
            }
          ])
          actions.push({
            label: 'Register',
          ...
          if (currentUser) {
            ...
            setConversation((prev) => [
              ...prev,
              {
                role: 'assistant',
                content:
                  "To complete your profile, you'll need to create an account first. Would you like to register now?"
              }
            ])
            setNextActions([
              {
  ...
  return (
    <div className='bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden relative'>
      ...
      <div className='p-6 relative z-10'>
        ...
        {showRoleSwitcher && (
          <div className='mb-4'>
            ...
            <div className='flex space-x-2'>
              <Button
                variant={inferredRole === 'brand' ? 'primary' : 'outline'}
                onClick={() => handleSwitchRole('brand')}
                className='flex items-center'
                techEffect={inferredRole === 'brand'}
              >
                <BuildingIcon className='h-4 w-4 mr-2' />
                Brand / Sponsor
              </Button>
              <Button
                variant={inferredRole === 'organizer' ? 'primary' : 'outline'}
                onClick={() => handleSwitchRole('organizer')}
                className='flex items-center'
                techEffect={inferredRole === 'organizer'}
              >
                <CalendarIcon className='h-4 w-4 mr-2' />
                Event Organizer
              </Button>
              <Button
                variant={inferredRole === 'community' ? 'primary' : 'outline'}
                onClick={() => handleSwitchRole('community')}
                className='flex items-center'
                techEffect={inferredRole === 'community'}
              >
        ...
        {draftProfile && Object.keys(draftProfile).length > 0 && (
          <div className='mt-6'>
          ...
          <div className='mt-6 flex flex-wrap gap-3'>
            {nextActions.map((action, index) => (
              <Button
                ...
                className='flex items-center'
                techEffect={action.primary}
              >
                {action.label.includes('Save') && (
                  <SaveIcon className='h-4 w-4 mr-2' />
                )}
                {action.label.includes('Complete') && (
