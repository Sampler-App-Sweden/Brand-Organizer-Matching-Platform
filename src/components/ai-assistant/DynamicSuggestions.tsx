...
export function DynamicSuggestions({
  ...
  useEffect(() => {
    const loadSuggestions = async () => {
      if (!draftProfile) {
        ...
        setSuggestions([
          "I'm looking for brands to sponsor my event",
          'I want to find events where I can sample my product',
          "I'd like to join product test panels",
          "I'm organizing a music festival and need sponsors"
        ])
        return
      }
      setIsLoading(true)
      try {
        const newSuggestions = await generateSuggestions(
          draftProfile,
          inferredRole,
          // Add a suitable third argument, e.g. an empty object or appropriate value
          {}
        )
  ...
  const visibleSuggestions =
    suggestions.length > 0
      ? [
          suggestions[rotationIndex % suggestions.length],
          suggestions[(rotationIndex + 1) % suggestions.length],
          suggestions[(rotationIndex + 2) % suggestions.length]
        ]
      : []
